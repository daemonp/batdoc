...
ocrs = { version = "0.12", default-features = false }
rten = { version = "0.24", default-features = false }
ureq = "2"
```

**Pin note (verified against the published ocrs 0.12.2 crates.io manifest):** the published ocrs has NO `rten` feature flag (features: `onnx` only) and hard-depends on `rten 0.24.0` with `features = ["rten_format"]`, default-features off. So do NOT set `features = ["rten"]` on ocrs — `rten_format` arrives via feature union from ocrs' own dependency. `rten` is pinned to 0.24 to match ocrs' published pin exactly, so Cargo unifies to one instance (0.x minor versions don't unify). `lopdf` and `ureq` already exist in the tree via `pdf-extract` (verified: pdf-extract 0.10 depends on `lopdf ^0.38` and `ureq ^2.6.2`), so nothing new enters the lockfile graph.
...
**Merged-state note (2026-08-13):** the extraction-fidelity merge landed since this plan was written. `parse_docx` now returns `Result<DocxOutput>` — a tuple struct destructured as `(blocks, image_defs, comments, footnotes, endnotes)` — and `extract_plain`/`extract_markdown` append extras via `append_extras_plain`/`append_extras_markdown`; `Run` gained a `marker: Option<NoteMarker>` field (constructors `Run::text`, `Run::footnote_ref`); table/cell parsers gained `footnotes`/`endnotes` index params. `Block::Image { markdown: String }` and `resolve_images`'s shape are UNCHANGED. Apply these exact deltas:

Change `extract_plain` (keep the existing body structure; add the param and pass it through):
...
Change `extract_markdown` — update the doc comment to mention OCR, add the param, and pass it through (keep the existing `image_defs`/extras ordering verbatim):
...
Change `parse_docx` — only the signature, the `image_rels` condition, and the `resolve_images` call (everything else — comments/footnotes/endnotes reads, `parse_body` call, `DocxOutput` construction — stays verbatim):
...
**Merged-state note (2026-08-13):** the extraction-fidelity merge added `Slide.notes: Vec<ShapeText>` (with `load_slide_notes`, `append_notes_markdown`, `append_notes_plain`, `render_shape_markdown`, `render_shape_plain` helpers). `parse_pptx` still returns `(Vec<Slide>, Vec<String>)` and the pic/image loop is unchanged. Apply these exact deltas:

Change `extract_plain` / `extract_markdown` (bodies unchanged apart from the param):
...
Change the `image_rels` condition inside the slide loop:
...
Replace the `let images = if extract_images && !image_rels.is_empty() { ... } else { Vec::new() };` block with:
...
(Keep the existing `image_counter`/`all_image_defs` declarations and the `num` loop variable; only the block above changes.)
...
Every `Slide { ... }` literal in `mod tests` gains `image_ocr: Vec::new(),` alongside the existing `notes: Vec::new(),`. The compiler lists any missed sites.
...
Expected: no duplicate versions (rten exactly 0.24.x once — matching ocrs' published pin); binary grows by the linked engine (models are NOT in the binary).
